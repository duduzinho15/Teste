"""
Scraper do Pelando para o sistema Garimpeiro Geek.
Coleta ofertas em tempo real da comunidade Pelando.
"""
...
class PelandoScraper:
    ...
    async def scrape_offers(self, max_offers: int = 100) -> List[Offer]:
        ...
        """
        try:
            self.logger.info(f"🕷️ Iniciando coleta de ofertas do Pelando (máx: {max_offers})")
            ...
            while len(offers) < max_offers:
                ...
                for pelando_offer in page_offers:
                    if len(offers) >= max_offers:
                        break
                    
                    # Converter para modelo Offer
                    offer = await self._convert_to_offer(pelando_offer)
            ...
            self.logger.error(f"❌ Erro na coleta do Pelando: {e}")
            self.stats["errors"] += 1
            return []
    
    async def _scrape_page(self, page: int) -> List[PelandoOffer]:
        """
        ...
        try:
            ...
            # - Retornar lista de PelandoOffer
    ...
    async def _convert_to_offer(self, pelando_offer: PelandoOffer) -> Optional[Offer]:
        """
        Converte uma oferta do Pelando para o modelo Offer.
        
        Args:
            pelando_offer: Oferta do Pelando
        ...
        try:
            ...
            if not pelando_offer.title or not pelando_offer.url:
                return None
            
            # Converter preços para Decimal
            price = Decimal(str(pelando_offer.price))
            original_price = None
            if pelando_offer.original_price:
                original_price = Decimal(str(pelando_offer.original_price))
            
            # Calcular desconto se não fornecido
            discount_percentage = pelando_offer.discount
            ...
            # Criar oferta
            offer = Offer(
                title=pelando_offer.title,
                price=price,
                url=pelando_offer.url,
                store=pelando_offer.store,
                original_price=original_price,
                discount_percentage=discount_percentage,
                category=pelando_offer.category,
                image_url=pelando_offer.image_url,
                source="pelando",
                scraped_at=pelando_offer.posted_at
            )
...
async def scrape_pelando_offers(max_offers: int = 100) -> List[Offer]:
    """
    Função de conveniência para coletar ofertas do Pelando.
...
if __name__ == "__main__":
    # Teste básico
    async def test():
        offers = await scrape_pelando_offers(5)
